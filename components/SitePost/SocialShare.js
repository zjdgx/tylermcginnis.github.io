...
export default function SocialShare ({type, tweet, url}) {
  if (type === 'facebook') {
    return (
      <a className="resp-sharing-button__link" href={`https://facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`} target="_blank" aria-label="">
    ...
    return (
      <a style={{marginLeft: 0}} className="resp-sharing-button__link" href={`https://twitter.com/intent/tweet/?text=${encodeURIComponent(tweet)}&amp;url=${encodeURIComponent(url)}`} target="_blank" aria-label="">
